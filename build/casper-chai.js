  ...
  module.exports = function(_chai) {
    ...
    _get_attrs = function(selector, attr) {
      ...
      fn = function(selector, _attr) {
        var _casper_chai_elements;
        _casper_chai_elements = __utils__.findAll(selector);
        return Array.prototype.map.call(_casper_chai_elements, function(e) {
          return e.getAttribute(_attr);
        });
      };
      return casper.evaluate(fn, {
        _selector: selector,
        _attr: attr
      });
    };
    ...
    */
    ...
    _addMethod('attrAny', function(attr) {
      ...
      selector = this._obj;
      attrs = _get_attrs(selector, attr);
      return this.assert(attrs.some(function(a) {
        return a;
      }), ("Expected one element matching selector " + selector + " to have attribute") + ("" + attr + " set, but none did"), ("Expected no elements matching selector " + selector + " to have attribute") + ("" + attr + " set, but at least one did"));
    });
    /*
    ...
    */
    ...
    _addMethod('attrAll', function(attr) {
      ...
      selector = this._obj;
      attrs = _get_attrs(selector, attr);
      return this.assert(attrs.every(function(a) {
        return a;
      }), ("Expected all elements matching selector " + selector + " to have attribute") + ("" + attr + " set, but one did not have it"), ("Expected one elements matching selector " + selector + " to not have ") + (" attribute " + attr + " set, but they all had it"));
    });
    /*
      @@@@ fieldValue
    ...
    */
    ...
    _addMethod('matchOnRemote', function(matcher) {
      ...
      if (typeof matcher !== 'string' && !(matcher instanceof RegExp)) {
        if (toString.call(remoteValue) === "[object RuntimeArray]") {
          remoteValue = Array.prototype.slice.call(remoteValue);
        }
        this._obj = remoteValue;
        return this.eql(matcher, remoteValue);
      } else {
    ...
    */
    ...
    _addMethod('tagName', function(ok_names) {
      ...
      tagnames = casper.evaluate(function(selector) {
        var elements;
      ...
      diff = tagnames.filter(function(t) {
    ...
    */
    ...
    return _addProperty('visible', function() {
      ...
      selector = this._obj;
      expect(selector).to.be.inDOM;
      return this.assert(casper.visible(selector), 'expected selector #{this} to be visible, but it was not', 'expected selector #{this} to not be visible, but it was');
    });
  };
